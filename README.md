...
Place hooks in your project and move to them quickly.

Forked from [xavierchen0/hooks.nvim](https://github.com/xavierchen0/hooks.nvim).
...
5. Add native cursor position rememberance (via `remember_cursor_position` opt)
...
Manage the buffer list using `:lua require("hooks").list()` floating buffer.

The list is editable like a regular buffer. So move and delete items from it.

Use `:lua require("hooks").buffer(1)` to switch to a hooked buffer.
